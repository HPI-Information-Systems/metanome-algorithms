...
public class PruningGraph {
  ...
  protected final Map<ColumnCombinationBitset, List<ColumnCombinationBitset>>
      columnCombinationMap =
      new HashMap<>();
  ...
  /**
   * Adds the given ColumnCombination to the graph. The graph information is automatically pruned
   * and OVERFLOW objects are created when necessary.
   */
  public void add(ColumnCombinationBitset columnCombination) {
    ...
    for (int i = 0; i < columnCombination.getNSubsetColumnCombinations(initialKeyLength).size();
      ...
      this.addToKey(columnCombination.getNSubsetColumnCombinations(initialKeyLength).get(i),
                    columnCombination, initialKeyLength);
    }
  }

  protected void addToKey(ColumnCombinationBitset key, ColumnCombinationBitset columnCombination,
                          int keyLength) {
  ...
  protected void addToSubKey(ColumnCombinationBitset key, ColumnCombinationBitset columnCombination,
                             int keyLength) {
    for (int i = 0; i < columnCombination.getNSubsetColumnCombinations(keyLength).size(); i++) {
      this.addToKey(columnCombination.getNSubsetColumnCombinations(keyLength).get(i),
                    columnCombination, keyLength);
    }
  }

  /**
   * Returns true iff the given columnCombination is a subset(negative graph) or superset(positive
   * graph). If true is returned the given columnCombination is already pruned.
   */
   ...
   */
  protected boolean findRecursively(ColumnCombinationBitset columnCombination,
