...
public class Sampler {
	...
	private de.metanome.algorithms.cfdfinder.structures.FDTree posCover;
	private int[][] compressedRecords;
	private List<de.metanome.algorithms.cfdfinder.structures.PositionListIndex> plis;
	...
	public Sampler(FDSet negCover, de.metanome.algorithms.cfdfinder.structures.FDTree posCover, int[][] compressedRecords, List<de.metanome.algorithms.cfdfinder.structures.PositionListIndex> plis, float efficiencyThreshold, ValueComparator valueComparator, MemoryGuardian memoryGuardian) {
	...
	public FDList enrichNegativeCover(List<de.metanome.algorithms.cfdfinder.structures.IntegerPair> comparisonSuggestions) {
		...
		if (this.attributeRepresentants == null) { // if this is the first call of this method
			de.metanome.algorithms.cfdfinder.utils.Logger.getInstance().write("Sorting clusters ...");
			long time = System.currentTimeMillis();
			ClusterComparator comparator = new ClusterComparator(this.compressedRecords, this.compressedRecords[0].length - 1, 1);
			for (de.metanome.algorithms.cfdfinder.structures.PositionListIndex pli : this.plis) {
				for (IntArrayList cluster : pli.getClusters()) {
					Collections.sort(cluster, comparator);
				}
				comparator.incrementActiveKey();
			}
			de.metanome.algorithms.cfdfinder.utils.Logger.getInstance().writeln("(" + (System.currentTimeMillis() - time) + "ms)");
		
			de.metanome.algorithms.cfdfinder.utils.Logger.getInstance().write("Running initial windows ...");
			...
			if (!this.queue.isEmpty())
				this.efficiencyThreshold = Math.min(this.efficiencyThreshold / 2, this.queue.peek().getEfficiency() * 0.9f); // This is an optimization that we added after writing the CFDFinder paper
		}
		
		de.metanome.algorithms.cfdfinder.utils.Logger.getInstance().writeln("Moving window over clusters ... ");
		...
		de.metanome.algorithms.cfdfinder.utils.Logger.getInstance().writeln(windows.toString());
	...
	*/		float sumNewNonFds = this.numNewNonFds.getInt(index);
		...
		public AttributeRepresentant(List<IntArrayList> clusters, FDSet negCover, de.metanome.algorithms.cfdfinder.structures.FDTree posCover, Sampler sampler, MemoryGuardian memoryGuardian) {
