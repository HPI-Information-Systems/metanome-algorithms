...
public class DVLogLog extends DVLogLogAlgorithm implements BasicStatisticsAlgorithm, RelationalInputParameterAlgorithm, StringParameterAlgorithm  {
	...
	public void setRelationalInputConfigurationValue(String identifier, RelationalInputGenerator... values) throws AlgorithmConfigurationException {
		if (!DVLogLog.Identifier.INPUT_GENERATOR.name().equals(identifier))
			throw new AlgorithmConfigurationException("Input generator does not match the expected identifier: " + identifier + " (given) but " + DVLogLog.Identifier.INPUT_GENERATOR.name() + " (expected)");
	...
	@Override
	  public void setStringConfigurationValue(String identifier, String... values)
	    ...
	    if (DVLogLog.Identifier.STANDARD_ERROR.name().equals(identifier))
	    {
  ...
  public String getDescription() {
    return "LogLog Cardinality Estimator. Durand, M., & Flajolet, P. (2003). Loglog counting of large cardinalities. In Algorithms-ESA 2003 (pp. 605-617). Springer Berlin Heidelberg";
  }
